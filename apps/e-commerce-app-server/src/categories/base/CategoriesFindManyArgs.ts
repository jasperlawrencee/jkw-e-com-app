/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CategoriesWhereInput } from "./CategoriesWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { CategoriesOrderByInput } from "./CategoriesOrderByInput";

@ArgsType()
class CategoriesFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => CategoriesWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => CategoriesWhereInput, { nullable: true })
  @Type(() => CategoriesWhereInput)
  where?: CategoriesWhereInput;

  @ApiProperty({
    required: false,
    type: [CategoriesOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [CategoriesOrderByInput], { nullable: true })
  @Type(() => CategoriesOrderByInput)
  orderBy?: Array<CategoriesOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { CategoriesFindManyArgs as CategoriesFindManyArgs };
