...
function Login() {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch('/api/login', { // Add the correct URL here
        method: 'POST',
        body: formData,
      });
        ...
        <div className="flex flex-col justify-between h-full">
          ...
          <div className="px-[3rem] flex text-center justify-center text-zinc-800">
            <form onSubmit={onSubmit} className='w-full flex flex-col space-y-[0.625rem]'>
              ...
              <input placeholder='Email' type="text" name="email" className='px-[1rem] py-[0.5rem] bg-transparent border-solid border-2 rounded-[0.375rem] outline-none' />
              <input placeholder='Password' type="password" name="password" className='px-[1rem] py-[0.5rem] bg-transparent border-solid border-2 rounded-[0.375rem] outline-none' />
              ...
              <button type='button' onClick={handleGoogle} className='bg-zinc-800 px-[1rem] py-[0.5rem] rounded-[0.375rem] text-slate-50 flex items-center justify-center gap-x-[0.625rem]'>
                <SiGoogle />
